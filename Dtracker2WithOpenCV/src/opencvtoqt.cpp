...
//template<typename Policy>
//template<typename Image>
//cv::Mat qimage_to_mat<Policy>::run(qimage_to_mat::Image img, bool swap)
//{
//    if(img.isNull()){
//        return cv::Mat();
//    }
 ...
 *@param swap : true : swap RGB to BGR; false, do nothing
 */
QImage mat_to_qimage_cpy(cv::Mat &mat, bool swap)
{
    if(!mat.empty()){
        switch(mat.type()){
 ...
 *@param swap : true : swap RGB to BGR; false, do nothing
 */
cv::Mat qimage_to_mat_cpy(QImage &img, bool swap)
{
    ...
    switch (img.format()) {
    case QImage::Format_RGB888:{
        cv::Mat result = cv::Mat(img.height(), img.width(), CV_8UC3, img.bits(), img.bytesPerLine());
        if(swap){
            cv::cvtColor(result, result, CV_RGB2BGR);
        }
        return result;
    ...
    case QImage::Format_Indexed8:{
        return cv::Mat(img.height(), img.width(), CV_8U, img.bits(), img.bytesPerLine());
    }
    case QImage::Format_RGB32:
    case QImage::Format_ARGB32:
    case QImage::Format_ARGB32_Premultiplied:{
        return cv::Mat(img.height(), img.width(), CV_8UC4, img.bits(), img.bytesPerLine());
    }
    default:
        break;
    }
